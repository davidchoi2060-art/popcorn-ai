...
■ ⚠ coords.json의 `cx_pct`/`cy_pct`를 그대로 그림 HTML의 `--x`/`--y`에 옮기지 마라
  (결함④, 2026-08-29 — 새 매뉴얼 5개·259개 핀 중 6곳이 이렇게 났다). 이 모듈이 재는
  것은 «요소 자신»의 상자일 뿐, 그 상자에 고정 크기 배지(실렌더 23~26px)를 놓았을 때
  그림 경계나 옆 배지와 겹치는지는 별개 문제다 — 특히 요소가 그림 가장자리에 가깝거나
  (제목·머리글처럼 위쪽에 있는 것) 다른 요소와 다닥다닥 붙어 있으면 정중앙 배치가 바로
  결함이 된다. 배치 기준·최소 여백 수치·왜 그런지는 `templates/admin/manual.html.j2`의
  `.manual-doc .pin` 규칙 바로 뒤 주석(결함④)에 있다 — 새 화면의 핀 좌표를 정할 때
  그 표를 먼저 본다. 다 정했으면 `scripts/manual_check_pins.py --slugs=<슬러그>`로
  4개 폭(1680·1192·768·480)에서 경계 이탈·상호 겹침이 0인지 확인한다.
