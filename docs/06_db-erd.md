...
## 21. [17차 개정] grid_cells 티어 경계 개정 — 2026 하반기 시장 반영 (2026-09-11, 마이그레이션 `0082`, **적용 대기 — DBA**)

`grid_cells`(0072)의 `budget_min`/`budget_max`(원, `[min, max)` 반열림)를 아래로 바꾼다. 티어 이름 7종·`cell_id`·`UNIQUE(tier, usage, platform)`은 그대로라 행 삭제·재생성 없이 UPDATE 만 한다(`grid_quotes.cell_id` FK 원장 보존).

| 티어 | 0072 (구) | **0082 (신)** |
|---|---|---|
| 팝콘 3 | 0 ~ 60만 | **0 ~ 150만** |
| 팝콘 5 | 60 ~ 100만 | **150 ~ 220만** |
| 팝콘 5+ | 100 ~ 150만 | **220 ~ 300만** |
| 팝콘 7 | 150 ~ 220만 | **300 ~ 400만** |
| 팝콘 7+ | 220 ~ 300만 | **400 ~ 550만** |
| 팝콘 9 | 300 ~ 450만 | **550 ~ 800만** |
