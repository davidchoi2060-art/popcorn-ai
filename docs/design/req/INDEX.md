...
| 9-2 | **팝콘AI 스토리**(고객 정보 허브 · 독립 섹션 — 게임·용도 정보, 이미지/영상, 팝콘AI 소개, 문의) | [req-customer-info-hub.md](req-customer-info-hub.md) | — | **정의서** 작성(2026-09-14 · 사장님 확정 "블로그·커뮤니티처럼 둘러보다 견적으로 진입" + 이름 「팝콘AI 스토리」 확정 + **첫 슬라이스 = 게임 정보 카드로 확정**, 이미지·영상 포함 요청 반영(④-B 신설) + **이미지·영상 저장소 = Cloudflare R2 확정**(W-7, 비용 조사 근거 GCS·NAS 대안 비교)). 화면 ID `CUS-STORY-010`(가칭) · 경로 미정. 게임 정보(설명·사양·견적)는 기존 `games`·`game_cell_map` 테이블로 즉시 착수 가능 — 이미지·영상은 `game_media` 신설 필요(구조는 `product_media` 선례, 저장소는 R2). `game_performance`(예상 프레임)는 실측 6행뿐이라 대부분 게임에서 비어 있음. 나머지(용도별 소개·팝콘AI 소개·문의 등록)는 콘텐츠 저작·원장 설계가 선행(W-1~W-4·W-6) |
