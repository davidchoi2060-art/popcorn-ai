...
flowchart LR
  ...
  S3 --> S2
  S2 --> S4["S4 장바구니<br/>FR-REC-040"]
  S3 --> S4
  S4 -->|"POST /api/orders"| S5["S5 주문 완료<br/>S5-010"]
  S4 --> MY1
...
| # | 막는 것 | 실측 | 푸는 방법 |
|---|---|---|---|
| ① | **화면이 인계를 부르지 않는다** | `mockups/` 전체에서 `handoff` 0건 | S4·S5 재작업 |
| ② | **운영 스위치가 자체 모드** | `GET /api/ops` → `pay: own` | 운영 전환 설정에서 `mall` |
...
| `ship` 배송 | `mall` | 몰 | 맞음 |
| `refund` 환불 | `mall` | 몰 | 맞음 |
| `pay` 결제 | **`own`** | 몰 | **어긋남 — 인계를 막는 것이 이것** |
| `member` 회원 | **`own`** | 몰 | **어긋남** |
| `settle` 정산 | `own` | — | **미정** · 팔지 않으면 정산이 없다. 사장님 판단 |
...
1. **S4 재작업** — `/api/orders` 대신 `/api/handoff`. S5 는 「주문 완료」가 아니라
   「몰로 넘김」이 된다.
