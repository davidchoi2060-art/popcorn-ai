...
REASON_LABELS = {
    ...
    # 2026-09-21 개정: 옛 `rec_gpu_parse_failed` 하나가 성격이 다른 셋을 뭉치고
    # 있었다(가벼운 게임 · 서열표 구멍 · 등급 미확정). 문구도 "대부분의 PC에서
    # 충분히 돌아갈 것으로 보이나" 라는 **추정**을 말하고 있었다 — 근거 없는
    # 낙관은 지어낸 사실과 같다. 셋으로 가르고, 가벼운 게임은 사유가 아니라
    # 매핑 경로로 옮겼다(tools/game_cell_mapper.py 머리 주석 「가벼운 게임」).
    "rec_gpu_not_in_ladder":
        "권장 사양 그래픽카드가 GPU 서열표에 없습니다 — 판매하지 않는 모델이라"
        " 서열 점수가 없고, 서열표에 추가하기 전까지는 판정할 수 없습니다",
    "rec_gpu_grade_unassigned":
        "권장 사양 그래픽카드가 GPU 서열표에 없고 게임 등급도 확정되지 않았습니다"
        " — 판정 근거가 둘 다 없습니다",
    "zero_matching_cells":
        "권장 사양을 충족하는 격자 칸이 현재 재고에 없습니다",
    # 옛 키 — DB 에 남아 있을 수 있어 사전에 남긴다(배치는 더 이상 쓰지 않는다).
    "rec_gpu_parse_failed":
        "권장 사양 그래픽카드를 GPU 서열표에서 찾지 못했습니다 — 낡은 사유 키입니다."
        " 매핑 배치를 다시 실행하면 정확한 사유로 갱신됩니다",
