...
■ 이번 범위 (2026-08-16 사장님 승인 -- "일단 제미나이부터", 같은 날 3사로 확장)
  이 모듈은 **호출하는 자리를 만드는 것까지**다. 어디에 쓸지(팝콘톡 S1 파싱 / S2 근거
  설명)는 다음 단계라 이 파일은 어떤 화면·엔드포인트에서도 아직 불려오지 않는다
  (import하는 곳 0곳 -- `api/main.py`의 라우터 자동 탐색이 이 모듈을 실어 보긴 하지만,
  module-level `router`가 없어 `_DISCOVERED_ROUTERS`에는 안 잡힌다).
