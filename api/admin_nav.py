...
■ 「재고 입고」 항목 신설 + href 연결 (2026-08-18, 기록자 · 9차 — 아래 그룹 주석을 뒤집는다)
  ⚠ **바로 아래 「매입 · 소싱」 그룹 주석의 「「재고 입고」만 뺐다 — 재고 원장은 쇼핑몰이
  갖는다」는 2026-08-11 판단이고, 근거가 사실과 달랐다는 것이 A-58 로 확정됐다**
  (2026-08-17 사장님 확정 · `docs/decisions/decision-log.md` A-58). 그 줄은 **지우지 않고**
  옆에 정정을 붙였다(CANON §5 — 왜 뒤집혔는지가 다음 판단의 재료다).

  **뒤집힌 근거(A-58 실측)**: `stock_movements` 는 우리 DB 에서 지금도 굴러가고
  (불변식 `stock_qty = SUM(qty_delta)` 위반 0건), `api/admin_stock.py` 세 갈래가 그 원장에
  쓰며, 추천 후보 조회가 `WHERE stock_qty > 0` 을 붙인다 — **재고가 곧 견적 품질이다.**
  두 곳이 세고 있지 않았다: 몰 재고는 주문·배송·환불의 근거이고 우리 원장은 «후보 자격»
  판정 신호다. **빠져 있던 것은 원장이 아니라 화면뿐이었다.**

  화면 ID `ADM-SRC-020`(구 화면의 기존 ID 유지 — 새 ID 를 주면 한 화면이 두 신원을 갖는다),
  경로 `/admin2/stock-inbound`.
  ⚠ **실측 형태는 다른 화면과 다르다**: `api/admin_ui_stock_inbound.py` 는
  `APIRouter(tags=["admin-ui"])`(prefix 없음) + `@router.get("/admin2/stock-inbound")` 로
  전체 경로를 한 줄에 적는다 — 앞 차수 노트들이 인용한 `APIRouter(prefix="/admin2")` +
  `@router.get("/<screen>")` 형태가 아니다. **최종 경로는 같다.** 다음 사람이 prefix 를
  찾다 「없으니 라우트가 없다」로 오판하지 않게 적어 둔다.

  **위치 — 그룹 끝.** IA 정본(`docs/design/admin-ia-draft-2026-08-09.md` 58줄)이 매입 축을
  공급처 → 매입 견적 → 단가표 반영 → **재고 입고** 순으로 적었고, 이 그룹의 현재 세 항목이
  이미 그 순서다. ID 순서(010 매입 견적 · 020 재고 입고 · 030 공급처)와는 어긋나지만
  **이 그룹은 처음부터 ID 순서가 아니었다** — 판매가 그룹에서와 같은 원칙을 이어 쓴다
  (기존 배치를 흔들지 않고 IA 문서 순서를 따른다).

  ⚠ **승인 디자인은 다른 자리를 그렸다 — 알고 고른 것이다(숨기지 않는다).**
  `docs/design/dc-stock-inbound.html:343` 의 `NAVL` 은 … 공급처 · 매입 견적(용산) ·
  **재고 입고** · 단가표 반영 … 순이고 `dc-stock-inbound-README.md` 도 「매입 견적과 단가표
  반영 «사이»」라 적었다. **그래도 IA 를 따랐다**: ① `CLAUDE.md` §LNB 동작 규격이
  「**항목 구성(IA)은 이 규격의 대상이 아니다** — 정본은 `api/admin_nav.py`」라고 못박았고
  ② 그 `NAVL` 은 실제로 우리 IA 와 다르다 — **「상품 분류 매핑」이 통째로 빠져 있다**
  (수를 적지 않는다: `NAVL` 을 세고 이 파일 `counts()["total"]` 과 비교하면 그때그때 안다).
  즉 원안의 사이드바는 IA 선언이 아니라 화면 맥락용 근사다. 계약(`docs/design/spec-stock-inbound.md:14`)은 그룹만 지정하고
  **순서를 말하지 않는다** — 계약이 침묵하는 자리라 IA 정본이 남는다.
  **한 줄로 되짚는 법**: `grep -n "NAVL=" docs/design/dc-stock-inbound.html` 로 원안 순서를
  보고, `grep -n "상품 분류 매핑" docs/design/dc-stock-inbound.html`(**0건** — 원안 LNB 가
  IA 전수가 아니라는 근거)와 대조한다. 사장님이 원안 자리를 원하시면 **아래 NAV 에서 한 줄을
  위로 옮기면 끝이고, 불변식(`counts()`·화면=메뉴)은 순서에 영향받지 않는다.**

  **연결 근거 — 확인자 3차 검증 «결함 0» + 회귀 전건 통과(하네스 통보 2026-08-18):**
      P-9103 라벨·사유 두 문장만 · `blocked` 행(SKU 39948) 정확
      **세 번째 누출 지점**(확정 결과 옆 사유) 해소 확인 — 「한 행이 두 원천으로 말하지
        않는다」(CLAUDE.md §화면 정직성)가 실측으로 지켜짐
      게이트 카드 이름표 둘 · 안전재고 사실 `title` 보존 · 카탈로그 `has_specs` 실값
      API↔DOM 문자 대조 9건 일치 · 성능 104~104ms · 치수 전 항목 불변
      쓰기 왕복 1건(상품 20489103 · log_id 9080)을 되돌려 원복, 재고 불변식 일치
      회귀 전건 통과(실패 0) — `REGRESSION_KEEP_SESSIONS` **없이** 돌려 평소 건너뛰던
        세션 왕복 항목까지 포함해 전부 실행했다

  ⚠ **이번은 `total` 도 함께 는다**(라벨 자체가 없던 화면이다 — 5·7차처럼 href 만 채우는
  경우가 아니다). 판매가 재산정(6차) 이후 두 번째 신설이다.

  확인법: `.venv/Scripts/python -c "from api.admin_nav import counts; print(counts())"`
  — `total` 39 -> 40 · `new` 39 -> 40 · `todo` 0 불변.
  그리고 `.venv/Scripts/python scripts/state.py` 의 §화면 「화면=메뉴」 줄이
  **「다르다 — 화면 40개 / LNB 39개」에서 「일치 (40)」로** 바뀐다 — 이 항목을 넣기 전에는
  라우트·템플릿만 있고 메뉴에 안 붙은 화면이 정확히 이 하나였다.

■ 「상품 등록」 항목 신설 — 관례를 앞당긴다 (2026-09-11, 사장님 확정 10차)
  ⚠ **이 프로젝트의 관례(1~9차)는 「확인자 재검증 통과 후 연결」이었다.** 이번은
  제작자 자기검증(콘솔 대체 정적 검사 — jinja2 렌더·node --check·curl 라운드트립)만
  끝난 시점에 사장님이 직접 "커밋하고, 메뉴 만들고, 검증해줘"로 **순서를 지정**했다 —
  검증은 이 연결 다음 단계다. 관례를 어긴 것이 아니라 **이번 지시가 순서를 명시했다.**
  화면 ID `ADM-PRD-011`, 경로 `/admin2/product-new`(`api/admin_ui_product_new.py`).
  위치는 「상품 관리」 바로 다음 — 목록에서 등록으로 넘어가는 도메인 순서 그대로다.
  ⚠ **결함이 나오면 이 링크부터 다시 닫는다** — 검증 전 연결이므로 되돌릴 수 있게
  이 노트를 남긴다.
"""
...
NAV = [
    ...
    #   ⚠ **위 한 줄은 2026-08-17 A-58 로 뒤집혔다 — 지우지 않고 남긴다(CANON §5).**
    #     걱정 자체는 타당했지만 **애초에 두 곳이 세고 있지 않았다**: 몰 재고는 주문·배송·
    #     환불(쇼핑몰 소관)의 근거이고 우리 `stock_movements` 는 «추천 후보 자격»(재고 0
    #     여부·안전재고 미달)을 판정하는 별개 신호다. 빠져 있던 것은 원장이 아니라 화면뿐.
    #     실측 근거·되짚는 법은 decision-log A-58 과 이 파일 docstring 9차 노트에 있다.
    ("매입 · 소싱", "truck", [
        ("공급처", "/admin2/suppliers", None),
        ("매입 견적(용산)", "/admin2/sourcing", None),
        ("단가표 반영", "/admin2/price-import", None),
        # A-58 로 재등재(2026-08-18). 위치는 IA 정본 순서 그대로 그룹 끝 —
        # `docs/design/admin-ia-draft-2026-08-09.md` 58줄.
        ("재고 입고", "/admin2/stock-inbound", None),
    ]),
