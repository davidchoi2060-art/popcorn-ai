...
■ href 연결 — 기록자, T-02 조건 충족 (2026-08-15)
  하네스의 「검증 통과」 통보(T-02 조건)를 받고 셋만 연다 — 조립 호환 규칙
  ...
  **다른 항목은 그대로 둔다** — href=None 인데 `api/admin_ui_*.py` 라우트 파일은 이미
  있는 화면이 여럿이다(부품 등급 관리·추천 가능 재고 현황 포함). 그건 「검증 통과
  통보」가 아직 없었거나(대부분) 라우트 파일이 지금 다른 작업으로 미커밋 수정 중이라
  (추천 가능 재고 현황) 열지 않았다 — 목록은 하네스 보고에 남긴다. 라벨은 이미
  정본이라 바꾸지 않았다(「추천 기준 보기」는 커밋 `a84fbc0`에서 이미 개명 완료).
"""
