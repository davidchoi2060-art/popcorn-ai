...
■ 왜 이 파일이 필요한가 (조사자 실측 · decision-log A-71)
  ...
  이 파일은 **1단계**만 다룬다. 슬롯별 후보는 화면에 박힌 값이 아니라 **실재고**에서
  뽑는다 — 모집단은 뷰 `v_recommendation_candidates` 하나뿐이고(`swap._pool_ctx` ->
  `recommend._load_pool` 경유, 아래 §재사용 참조), 슬롯당 `EXPERT_SAMPLE_N`(=5)개를
  가격 5~95백분위 구간에서 고르게 뽑는다(`_price_spread_sample`). 종류가 둘인 슬롯
  (COOLER — 공랭·수랭, `taxonomy.SLOT_TYPES`)은 종류별 최소 1개를 먼저 보장한 뒤
  나머지를 후보 수 비례로 배분한다(`_sample_slot`) — 표본 수·방식의 근거는
  `EXPERT_SAMPLE_N` 정의 옆 주석에 있다. 1단계가 새로 여는 것은 ① 이 읽기 API
  (`GET /slots`, 실재고 소수 후보) ② 고객이 고른 구성을 **서버 규칙 9행 전부**로
  판정하고 원장에 남기는 쓰기 API(`POST /build`) 둘이다.

  ⚠ **decision-log A-71(2026-08-19 사장님 확정)과 다르다 — 감추지 않고 적는다.** A-71
  은 "1단계 … 부품 목록은 **지금의 예시 21개를 그대로 유지**한다"고 확정했다. 그런데
  실제로 배포된 이 파일은 그 21개를 대체했다 — `GET /slots`가 위 실재고 표본을 내려
  주고, `mockups/mvp1/s1-session.html`(568행 부근)도 스스로 "예전엔 여기 7슬롯×3안=
  21개가 리터럴로 박혀 있었고 … 이제 서버가 실재고 표본을 준다"고 적어 같은 사실을
  확인해 준다. 즉 A-71 이 **2단계**(실재고 선택, 화면 개편 필요 —
  `docs/design/req/req-expert-part-picker.md`)로 미뤄 둔 일의 일부가 1단계 배포에
  이미 들어간 상태다. **이 차이의 처리(A-71 기록을 갱신할지 · 이 API 를 2단계의
  일부로 재분류할지)는 사장님 확인 대기다** — 그 정의서가 남긴 미정 **U-29**(모드로
  둘지 별도 화면으로 뗄지) · **U-30**(모집단을 후보 뷰로 좁힐지 판매중 전체로
  넓힐지)과 이어진다.
